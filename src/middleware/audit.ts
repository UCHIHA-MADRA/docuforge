...
function getEventTypeFromPath(pathname: string, method: string): AuditEventType {
  ...
  if (pathname.includes('/api/pdf/compress')) return AuditEventType.PDF_COMPRESSED;
  if (pathname.includes('/api/pdf/annotate')) return AuditEventType.PDF_ANNOTATED;
  ...
  if (pathname.includes('/api/spreadsheet')) {
    ...
    if (method === 'PUT' || method === 'PATCH') return AuditEventType.SPREADSHEET_EDITED;
  }
  
  // Theme operations
  if (pathname.includes('/api/theme')) return AuditEventType.THEME_CHANGED;
  if (pathname.includes('/api/settings')) return AuditEventType.SETTINGS_UPDATED;
...
function getRiskLevel(pathname: string, method: string, statusCode: number): RiskLevel {
  // Critical events
  if (statusCode === 401 || statusCode === 403) return 'HIGH';
  if (pathname.includes('/auth') && statusCode >= 400) return 'MEDIUM';
  
  // High-risk operations
  if (method === 'DELETE') return 'MEDIUM';
  if (pathname.includes('/admin')) return 'MEDIUM';
...
export const auditHooks = {
  // Document operations
  async logDocumentUpload(userId: string, documentId: string, filename: string, size: number) {
  ...
  async logDocumentDownload(userId: string, documentId: string, filename: string) {
    await auditLogger.logDocumentOperation(
  ...
  async logDocumentShare(userId: string, documentId: string, sharedWith: string[]) {
    await auditLogger.logDocumentOperation(
  ...
  async logPDFCompress(userId: string, documentId: string, originalSize: number, compressedSize: number) {
  ...
  // OCR operations
  async logOCRExtraction(userId: string, documentId: string, extractedTextLength: number, confidence: number) {
    await auditLogger.logDocumentOperation(
  ...
  // Spreadsheet operations
  async logSpreadsheetCreation(userId: string, spreadsheetId: string, name: string) {
    await auditLogger.logDocumentOperation(
  ...
  async logSpreadsheetEdit(userId: string, spreadsheetId: string, changes: SpreadsheetChanges) {
    await auditLogger.logDocumentOperation(
  ...
  // Security events
  async logUnauthorizedAccess(ipAddress: string, userAgent: string, attemptedResource: string) {
  ...
  async logSuspiciousActivity(userId: string | undefined, ipAddress: string, activity: string, details: Record<string, unknown>) {
    await auditLogger.logSecurityEvent(
  ...
  async logLoginAttempt(email: string, ipAddress: string, userAgent: string, success: boolean, userId?: string) {
  ...
  async logSettingsUpdate(userId: string, settingType: string, changes: Record<string, unknown>) {
    await auditLogger.log({
      eventType: AuditEventType.SETTINGS_UPDATED,
      userId,
      action: `Updated ${settingType} settings`,
      details: JSON.stringify({ settingType, changes, type: 'settings_update' }),
      riskLevel: RiskLevel.LOW,
      success: true
    });
  }
