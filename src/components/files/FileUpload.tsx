...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const validateFile = useCallback((file: File): string | null => {
    ...
    if (file.size > uploadConfig.maxFileSize) {
      return `File size exceeds maximum allowed size of ${uploadConfig.maxFileSize / (1024 * 1024)}MB`;
    }
    ...
    try {
      ...
      if (response.ok && result.success) {
        setFiles(prev => prev.map(f => 
          f.id === uploadFile.id 
            ? { ...f, status: 'completed', progress: 100, fileId: result.fileId }
            : f
        ));
        onUploadComplete?.(result.fileId);
        await auditHooks.logFileUpload(user.id, result.fileId, uploadFile.file.name, uploadFile.file.size);
      } else {
