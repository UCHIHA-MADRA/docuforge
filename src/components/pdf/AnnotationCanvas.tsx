...
export function AnnotationCanvas({
  ...
  }, [pageAnnotations, selectedAnnotation, selectionState, drawingState, selectedTool, isVisible]);

  const drawHighlight = (ctx: CanvasRenderingContext2D, annotation: HighlightAnnotation) => {
    ctx.fillStyle = `rgba(${annotation.color.r * 255}, ${annotation.color.g * 255}, ${annotation.color.b * 255}, ${annotation.opacity})`;
    ctx.globalCompositeOperation = annotation.blendMode as GlobalCompositeOperation;
  ...
  const drawComment = (ctx: CanvasRenderingContext2D, annotation: CommentAnnotation) => {
    const x = annotation.position.x * scale;
    const y = annotation.position.y * scale;
    const size = 20 * scale;

    // Draw comment icon background
    ctx.fillStyle = `rgb(${annotation.color.r * 255}, ${annotation.color.g * 255}, ${annotation.color.b * 255})`;
  ...
  const drawStamp = (ctx: CanvasRenderingContext2D, annotation: StampAnnotation) => {
    const x = annotation.position.x * scale;
    const y = annotation.position.y * scale;
    ...
    ctx.fillStyle = `rgba(${annotation.color.r * 255}, ${annotation.color.g * 255}, ${annotation.color.b * 255}, 0.1)`;
    ctx.fillRect(0, 0, width, height);

    // Draw stamp border
    ctx.strokeStyle = `rgb(${annotation.color.r * 255}, ${annotation.color.g * 255}, ${annotation.color.b * 255})`;
    ctx.lineWidth = 2;
  ...
  const drawDrawing = (ctx: CanvasRenderingContext2D, annotation: DrawingAnnotation) => {
    ctx.strokeStyle = `rgb(${annotation.color.r * 255}, ${annotation.color.g * 255}, ${annotation.color.b * 255})`;
  ...
  const drawText = (ctx: CanvasRenderingContext2D, annotation: TextAnnotation) => {
    ...
    if (annotation.backgroundColor) {
      ctx.fillStyle = `rgba(${annotation.backgroundColor.r * 255}, ${annotation.backgroundColor.g * 255}, ${annotation.backgroundColor.b * 255}, ${annotation.opacity})`;
      const metrics = ctx.measureText(annotation.content);
      ctx.fillRect(0, -annotation.fontSize * scale, metrics.width, annotation.fontSize * scale);
    }

    // Draw border if specified
    if (annotation.borderColor && annotation.borderWidth > 0) {
      ctx.strokeStyle = `rgb(${annotation.borderColor.r * 255}, ${annotation.borderColor.g * 255}, ${annotation.borderColor.b * 255})`;
      ctx.lineWidth = annotation.borderWidth * scale;
      const metrics = ctx.measureText(annotation.content);
      ctx.strokeRect(0, -annotation.fontSize * scale, metrics.width, annotation.fontSize * scale);
    }

    // Draw text
    ctx.fillStyle = `rgb(${annotation.color.r * 255}, ${annotation.color.g * 255}, ${annotation.color.b * 255})`;
    ctx.font = `${annotation.fontStyle} ${annotation.fontSize * scale}px ${annotation.fontFamily}`;
  ...
  const drawLink = (ctx: CanvasRenderingContext2D, annotation: LinkAnnotation) => {
  ...
  const drawSelectionBorder = (ctx: CanvasRenderingContext2D, annotation: Annotation) => {
      ...
      case 'highlight':
        ...
        if (highlight.rectangles.length > 0) {
          ...
          bounds = {
            ...
            width: Math.max(firstRect.x + firstRect.width, lastRect.x + lastRect.width) - Math.min(firstRect.x, lastRect.x),
            height: Math.max(firstRect.y + firstRect.height, lastRect.y + lastRect.height) - Math.min(firstRect.y, lastRect.y),
          };
        } else {
          return;
        }
  ...
  const drawSelectionRectangle = (ctx: CanvasRenderingContext2D) => {
    const startX = Math.min(selectionState.startPoint.x, selectionState.endPoint.x);
    const startY = Math.min(selectionState.startPoint.y, selectionState.endPoint.y);
    const width = Math.abs(selectionState.endPoint.x - selectionState.startPoint.x);
    const height = Math.abs(selectionState.endPoint.y - selectionState.startPoint.y);
  ...
  const drawCurrentPath = (ctx: CanvasRenderingContext2D) => {
    ...
    ctx.strokeStyle = `rgb(${currentColor.r * 255}, ${currentColor.g * 255}, ${currentColor.b * 255})`;
  ...
  const isPointInAnnotation = (point: Point, annotation: Annotation): boolean => {
    switch (annotation.type) {
      case 'highlight':
        const highlight = annotation as HighlightAnnotation;
        return highlight.rectangles.some(rect => 
          point.x >= rect.x * scale && point.x <= (rect.x + rect.width) * scale &&
          point.y >= rect.y * scale && point.y <= (rect.y + rect.height) * scale
        );
      case 'comment':
        const comment = annotation as CommentAnnotation;
        return point.x >= comment.position.x * scale && 
      ...
      case 'stamp':
        const stamp = annotation as StampAnnotation;
        return point.x >= stamp.position.x * scale && 
      ...
      case 'text':
        ...
        return point.x >= text.position.x * scale && 
      ...
      case 'link':
        ...
        return point.x >= link.rectangle.x * scale && 
    ...
    if (clickedAnnotation) {
      ...
      if (!clickedAnnotation.isLocked) {
        setDragState({
          ...
          dragOffset: {
            x: point.x - ((clickedAnnotation as CommentAnnotation | StampAnnotation | TextAnnotation).position?.x ?? 0) * scale,
            y: point.y - ((clickedAnnotation as CommentAnnotation | StampAnnotation | TextAnnotation).position?.y ?? 0) * scale,
          },
        });
      }
      return;
  ...
  const handleMouseMove = (event: React.MouseEvent) => {
    ...
    if (dragState.isDragging && dragState.draggedAnnotation) {
      ...
      annotationManager.updateAnnotation(dragState.draggedAnnotation.id, {
        position: newPosition,
      } as Partial<CommentAnnotation | StampAnnotation | TextAnnotation>);
  ...
  const handleMouseUp = (event: React.MouseEvent) => {
    ...
    if (selectionState.isSelecting && selectedTool === 'highlight') {
      const startX = Math.min(selectionState.startPoint.x, selectionState.endPoint.x) / scale;
      const startY = Math.min(selectionState.startPoint.y, selectionState.endPoint.y) / scale;
      const width = Math.abs(selectionState.endPoint.x - selectionState.startPoint.x) / scale;
      const height = Math.abs(selectionState.endPoint.y - selectionState.startPoint.y) / scale;

      if (width > 5 && height > 5) {
        const rectangles: Rectangle[] = [{ x: startX, y: startY, width, height }];
    ...
    if (drawingState.isDrawing && selectedTool === 'drawing') {
      if (drawingState.currentPath.length > 1) {
        const paths: DrawingPath[] = [{
          points: drawingState.currentPath.map(p => ({ x: p.x / scale, y: p.y / scale })),
        }];
  ...
  const handleCommentCreate = () => {
    if (dialogData.content?.trim()) {
      const commentId = annotationManager.createComment(
        ...
        dialogData.content,
        userId,
dialogData.iconType as 'comment' | 'note' | 'key' | 'help' | 'insert' | 'paragraph'
      );
  ...
  const handleTextCreate = () => {
    if (dialogData.content?.trim()) {
      const textId = annotationManager.addAnnotation({
        type: 'text',
        pageIndex,
  ...
  const handleLinkCreate = () => {
    if (dialogData.url?.trim()) {
      const linkId = annotationManager.addAnnotation({
        type: 'link',
        pageIndex,
        bounds: {
          x: dialogPosition.x,
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea
                ...
                onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setDialogData(prev => ({ ...prev, content: e.target.value }))}
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: string) => setDialogData(prev => ({ ...prev, iconType: value }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
      ...
      <Dialog open={showTextDialog} onOpenChange={setShowTextDialog}>
        <DialogContent>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDialogData(prev => ({ ...prev, fontSize: parseInt(e.target.value) }))}
                  min={8}
                  max={72}
                />
              </div>
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: string) => setDialogData(prev => ({ ...prev, fontFamily: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleTextCreate}>
              Add Text
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showLinkDialog} onOpenChange={setShowLinkDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDialogData(prev => ({ ...prev, url: e.target.value }))}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDialogData(prev => ({ ...prev, displayText: e.target.value }))}
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: string) => setDialogData(prev => ({ ...prev, linkType: value }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleLinkCreate}>
              Add Link
            </Button>
          </DialogFooter>
        </DialogContent>
