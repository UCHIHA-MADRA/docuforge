...
class PDFProcessor {
   ...
   */
  async createNewPDF(pageSize: 'A4' | 'Letter' | 'Legal' = 'A4'): Promise<PDFProcessingResult> {
    try {
  ...
  private async loadStandardFonts(): Promise<void> {
    ...
    const standardFonts = [
      ...
      { name: 'Helvetica-BoldOblique', font: StandardFonts.HelveticaBoldOblique },
  ...
  private extractMetadata(): PDFMetadata {
    ...
    return {
      ...
      keywords: this.pdfDoc.getKeywords()?.split(',').map(k => k.trim()) || [],
  ...
  async addTextElement(
    ...
    const fontName = this.getFontName(fontFamily, formatting.bold, formatting.italic);
   ...
   */
  async editTextElement(elementId: string, updates: Partial<TextElement>): Promise<boolean> {
   ...
   * Move text element
   */
  async moveTextElement(elementId: string, x: number, y: number): Promise<boolean> {
   ...
   */
  async formatTextElement(elementId: string, formatting: FormattingOptions): Promise<boolean> {
   ...
   */
  private getFontName(fontFamily: string, bold?: boolean, italic?: boolean): string {
   ...
   */
  private async drawTextElement(page: PDFPage, element: TextElement): Promise<void> {
    const fontName = this.getFontName(element.fontFamily, element.bold, element.italic);
    const font = this.fonts.get(fontName) || this.fonts.get('Helvetica')!;
    
    // Handle multi-line text
    const lines = this.wrapText(element.content, element.width, font, element.fontSize);
  ...
  private async redrawPage(pageIndex: number): Promise<void> {
    ...
    const pageElements = Array.from(this.textElements.values())
  ...
  async reorderPages(fromIndex: number, toIndex: number): Promise<boolean> {
    ...
    const pages = this.pdfDoc.getPages();
    if (fromIndex >= pages.length || toIndex >= pages.length || fromIndex < 0 || toIndex < 0) {
      return false;
    }
   ...
   * Rotate page
   */
  async rotatePage(pageIndex: number, degrees: 90 | 180 | 270): Promise<boolean> {
    if (!this.pdfDoc) {
      throw new Error('No PDF document loaded');
    }
    ...
    if (pageElements.length > 0) {
      ...
      pageElements.forEach(element => {
        ...
        const { x, y } = this.rotateCoordinates(element.x, element.y, width, height, degrees);
        element.x = x;
        element.y = y;
      });
   ...
   */
  private rotateCoordinates(x: number, y: number, pageWidth: number, pageHeight: number, degrees: number): { x: number; y: number } {
    switch (degrees) {
      case 90:
  ...
  async duplicatePage(pageIndex: number): Promise<number> {
    ...
    const elementsOnPage = Array.from(this.textElements.values())
      .filter(element => element.pageIndex === pageIndex);
  ...
  updateMetadata(metadata: Partial<PDFMetadata>): void {
    ...
    if (metadata.keywords) this.pdfDoc.setKeywords(metadata.keywords.join(', '));
    ...
    if (metadata.creationDate) this.pdfDoc.setCreationDate(metadata.creationDate);
    if (metadata.modificationDate) this.pdfDoc.setModificationDate(metadata.modificationDate);
  }
  ...
  searchText(query: string, caseSensitive = false): TextElement[] {
    ...
    return Array.from(this.textElements.values())
      .filter(element => {
        const content = caseSensitive ? element.content : element.content.toLowerCase();
