...
const STORAGE_BASE_PATH = process.env.ENCRYPTED_STORAGE_PATH || './storage/encrypted';
 ...
 */
function generateSecureFilePath(userId: string, fileId: string, category: string = 'documents'): string {
  const userHash = crypto.createHash('sha256').update(userId).digest('hex').substring(0, 8);
  const fileHash = crypto.createHash('sha256').update(fileId).digest('hex').substring(0, 16);
...
export async function storeEncryptedFile(
  ...
  try {
    ...
    const category = metadata.mimeType.startsWith('image/') ? 'images' : 'documents';
    ...
    throw new Error(`Failed to store encrypted file: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function retrieveEncryptedFile(
  ...
  try {
    ...
    try {
      const storedData = JSON.parse(fileRecord.metadata || '{}');
      ...
      checksum = crypto.createHash('sha256').update(decryptedBuffer).digest('hex');
    ...
    return {
      ...
      metadata: JSON.parse(metadataJson || '{}') as FileMetadata,
    };
  } catch (error) {
    throw new Error(`Failed to retrieve encrypted file: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function deleteEncryptedFile(
  ...
  try {
    ...
    try {
      ...
      console.warn(`Could not delete file from disk: ${fileRecord.path}`, deleteError);
    }
    ...
    throw new Error(`Failed to delete encrypted file: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function listUserFiles(
...
): Promise<{ files: EncryptedFileRecord[]; total: number; page: number; limit: number }> {
  ...
  const fileRecords: EncryptedFileRecord[] = files.map(file => {
    ...
    try {
      storedData = JSON.parse(file.metadata || '{}');
    } catch {
    ...
    return {
      ...
      encryptedMetadata: storedData.encryptedMetadata ? JSON.parse(storedData.encryptedMetadata) : {} as EncryptedData,
      checksum: storedData.checksum || '',
      size: file.size,
...
export async function getStorageStats(userId?: string): Promise<StorageStats> {
  ...
  return {
    ...
    userSize: userStats?._sum.size || 0,
...
export async function createEncryptedBackup(
  ...
  try {
    ...
    const backupKey = generateDocumentKey(`backup-${userId}-${Date.now()}`, backupPassword);
    const encryptedBackup = encryptFile(Buffer.from(backupJson), backupKey, true);
    
    // Save backup to disk
    const backupPath = path.join(STORAGE_BASE_PATH, 'backups', `${userId}-${Date.now()}.backup`);
