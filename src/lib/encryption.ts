 ...
 */
export function deriveKeyFromPassword(password: string, salt?: Buffer): EncryptionKey {
  const keySalt = salt || crypto.randomBytes(SALT_LENGTH);
  const key = crypto.pbkdf2Sync(password, keySalt, PBKDF2_ITERATIONS, KEY_LENGTH, 'sha256');
 ...
 */
export function deriveKeyFromPasswordScrypt(password: string, salt?: Buffer): EncryptionKey {
  const keySalt = salt || crypto.randomBytes(SALT_LENGTH);
 ...
 */
export function encryptData(data: string | Buffer, encryptionKey: EncryptionKey, useScrypt = false): EncryptedData {
  try {
 ...
 */
export function decryptData(encryptedData: EncryptedData, password?: string, key?: Buffer): string {
  try {
    ...
    if (key) {
      ...
      if (encryptedData.keyDerivation === 'scrypt') {
        decryptionKey = crypto.scryptSync(password, salt, KEY_LENGTH, SCRYPT_OPTIONS);
      } else {
        decryptionKey = crypto.pbkdf2Sync(password, salt, PBKDF2_ITERATIONS, KEY_LENGTH, 'sha256');
      }
    } else {
    ...
    (decipher as crypto.CipherGCM).setAuthTag(tag);
 ...
 */
export function encryptFile(fileBuffer: Buffer, encryptionKey: EncryptionKey, useScrypt = false): EncryptedData {
 ...
 */
export function decryptFile(encryptedData: EncryptedData, password?: string, key?: Buffer): Buffer {
...
export function verifyPassword(password: string, hash: string, salt: string): boolean {
  try {
    ...
    const computedHash = crypto.pbkdf2Sync(password, saltBuffer, PBKDF2_ITERATIONS, KEY_LENGTH, 'sha256');
 ...
 */
export function encryptForTransmission(data: string | Buffer | object, password: string): string {
 ...
 */
export function decryptFromTransmission<T>(encryptedPayload: string, password: string): T {
  const encryptedData: EncryptedData = JSON.parse(Buffer.from(encryptedPayload, 'base64').toString());
 ...
 */
export function generateDocumentKey(documentId: string, userKey: string): EncryptionKey {
  const combinedKey = `${documentId}:${userKey}`;
  const salt = crypto.createHash('sha256').update(documentId).digest();
  const key = crypto.pbkdf2Sync(combinedKey, salt, PBKDF2_ITERATIONS, KEY_LENGTH, 'sha256');
...
export function createEncryptedDocument(
  ...
  const encryptedMetadata = encryptData(JSON.stringify(metadata), documentKey, true);
  ...
  if (privateKey) {
    const dataToSign = JSON.stringify({
      id: documentId,
      contentHash: crypto.createHash('sha256').update(encryptedContent.data).digest('hex'),
      metadataHash: crypto.createHash('sha256').update(encryptedMetadata.data).digest('hex'),
    });
...
export function decryptDocument(
  ...
  userKey: string,
  publicKey?: string
): { content: string | Buffer; metadata: { [key: string]: unknown }; verified: boolean } {
  // Verify signature if public key is provided
  let verified = false;
  if (publicKey && encryptedDocument.signature) {
    const dataToVerify = JSON.stringify({
      id: encryptedDocument.id,
      contentHash: crypto.createHash('sha256').update(encryptedDocument.encryptedContent.data).digest('hex'),
      metadataHash: crypto.createHash('sha256').update(encryptedDocument.encryptedMetadata.data).digest('hex'),
    });
    verified = verifySignature(dataToVerify, encryptedDocument.signature, publicKey);
  }
  
  const documentKey = generateDocumentKey(encryptedDocument.id, userKey);
  
  const content = decryptData(encryptedDocument.encryptedContent, undefined, documentKey.key);
  const metadataJson = decryptData(encryptedDocument.encryptedMetadata, undefined, documentKey.key);
