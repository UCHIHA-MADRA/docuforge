...
import { z } from 'zod';
import PDFProcessor, { FormattingOptions, PDFEditOperation } from '@/lib/pdf-processor';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const canEdit = 
      ...
      (file.document && await checkPermission(
        session.user.id,
        'document:edit',
        { documentId: file.document.id }
      )) ||
      await checkPermission(session.user.id, 'admin:file_management');
    ...
    if (!result.success) {
      return NextResponse.json({ error: result.error || 'Failed to load PDF' }, { status: 500 });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (operation) {
      case 'create': {
        const { title, author, organizationId } = CreatePDFSchema.parse(body);
        ...
        const file = await prisma.file.create({
          data: {
            ...
            size: 0, // Will be updated when saved
            userId: session.user.id,
            encryptionKey: '', // Will be set when saved
      ...
      case 'add_text': {
        const { fileId, content, x, y, pageIndex, formatting } = AddTextElementSchema.parse(body);
        ...
        const elementId = await processor.addTextElement(content, x, y, pageIndex, formatting);
      ...
      case 'edit_text': {
        const { fileId, elementId, updates } = EditTextElementSchema.parse(body);
        ...
        if (!success) {
          return NextResponse.json({ error: 'Failed to edit text element' }, { status: 500 });
        }
      ...
      case 'delete_text': {
        ...
        if (!success) {
          return NextResponse.json({ error: 'Failed to delete text element' }, { status: 500 });
        }
      ...
      case 'batch_operations': {
        ...
        if (!success) {
          return NextResponse.json({ error: 'Failed to apply batch operations' }, { status: 500 });
        }
      ...
      case 'page_operation': {
        const { fileId, operation: pageOp, pageIndex, rotation } = PageOperationSchema.parse(body);
        ...
        switch (pageOp) {
          case 'add':
            success = await processor.addPage();
            break;
          case 'delete':
        ...
        if (!success) {
          return NextResponse.json({ error: `Failed to ${pageOp} page` }, { status: 500 });
        }
      ...
      case 'save': {
        ...
        if (!result.success || !result.data) {
          return NextResponse.json({ error: result.error || 'Failed to save PDF' }, { status: 500 });
        }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const processor = new PDFProcessor();
    const success = await processor.updateMetadata(metadata);
